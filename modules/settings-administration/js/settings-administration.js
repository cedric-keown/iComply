...
async function handleAddUserProfile(e) {
    ...
    try {
        ...
        // No UUID validation needed - it will be auto-generated by the database
        ...
        if (isEditMode) {
            ...
            // Create new - UUID will be auto-generated by database
            const userData = {
                role_id: roleId,
                first_name: firstName,
                last_name: lastName,
            ...
            // Only include ID if one was provided (e.g., linking to existing auth.users)
            if (userId && userId.trim()) {
                userData.id = userId;
            }
            
            result = await dataFunctions.createUserProfile(userData);
            
            // Link to representative if selected
            if (result && result.success && result.id && selectedRepId) {
                const linkResult = await updateRepresentativeLink(result.id, selectedRepId);
                console.log('Representative link result:', linkResult);
            }
        }
